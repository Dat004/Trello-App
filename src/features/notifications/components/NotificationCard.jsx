...
const INVITE_TYPES = ["member_invited"];
...
function NotificationCard({
  ...
  const handleRespond = (action, notification) => {
    setResponded(action);
    if (onRespond) onRespond({ action, notification });
  };
    ...
    <Card className={`transition-all hover:shadow-md ${!notification.is_read ? "border-primary/50 bg-primary/5 shadow-sm" : "bg-card"}`}>
      <CardContent className="p-4">
        <div className="flex flex-col gap-3">
          ...
          {/* Invite Actions — separate row below */}
          {isInvite && (
            <div className="ml-14 pt-2 border-t border-border">
              {responded ? (
                <div className={`flex items-center gap-2 text-sm font-medium ${
                  responded === 'accept' ? 'text-green-600' : 'text-muted-foreground'
                }`}>
                  <CheckCheck className="h-4 w-4" />
                  {responded === 'accept' ? 'Đã đồng ý tham gia' : 'Đã từ chối lời mời'}
                </div>
              ) : (
